...
class ExtensionsFetcher:
    ...
    async def execute(self) -> ExtensionsFetchResult:
        ...
        for extension_name in extension_name_to_version_dict:
            extension_active_version = extension_name_to_version_dict[extension_name]
            extension_configuration = await self.get_extension_configuration_from_cache_or_download(
                extension_name, extension_active_version
            )
            services = [service.get("service") for service in extension_configuration.get("gcp")]
