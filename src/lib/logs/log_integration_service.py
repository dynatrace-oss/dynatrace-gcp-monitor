...
class LogIntegrationService:
    ...
    async def close(self):
        ...
        for attr in ("gcp_session", "dt_session"):
            ...
            if session is not None and not session.closed:
                sessions_to_close.append(session.close())
    ...
    async def perform_pull(
        ...
        tasks_to_pull_messages = [
            self.gcp_client.pull_messages(logging_context, self.gcp_session)
