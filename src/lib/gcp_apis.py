...
# This function return all the disabled projects that Service Account has access to
# and all disabled apis of the enabled projects
async def get_disabled_projects_and_disabled_apis_by_project_id(
        context, projects_ids) -> Tuple[Set[str], Dict[str, Set[str]]]:
...
async def _check_if_project_is_disabled_and_get_disabled_api_set(
        context: MetricsContext, project_id: str) -> Tuple[str, bool, Set[str]]:
    try:
...
async def _check_x_goog_user_project_header_permissions(context: MetricsContext, project_id: str):
    ...
    if resp.status == 200:
        ...
        context.use_x_goog_user_project_header[project_id] = False
        context.log(project_id, "Ignoring destination SERVICE_USAGE_BOOKING. "
                                "Missing permission: 'serviceusage.services.use'")
    else:
...
async def _get_all_disabled_apis(context: MetricsContext, project_id: str):
    ...
    try:
        ...
        context.log(project_id, f'Cannot get disabled APIs: '
                                f'{_GCP_SERVICE_USAGE_URL}/projects/{project_id}/services?filter=state:DISABLED. {e}')
        return disabled_apis
