...
async def query_metrics(execution_id: Optional[str], services: Optional[List[GCPService]] = None):
    ...
    async with init_gcp_client_session() as gcp_session, init_dt_client_session() as dt_session:
        ...
        process_project_metrics_tasks = [
            process_project_metrics(context, project_id, services, disabled_apis_by_project_id.get(project_id, set()), excluded_metrics_and_dimensions)
            for project_id
            in projects_ids
        ]
...
async def process_project_metrics(context: MetricsContext, project_id: str, services: List[GCPService],
    ...
    try:
        ...
        ingest_lines = await fetch_ingest_lines_task(context, project_id, services, disabled_apis, excluded_metrics_and_dimensions)
        ...
        for metric in service.metrics:
            ...
            if not metric.autodiscovered_metric or project_id in metric.project_ids:
                ...
                fetch_metric_coro = run_fetch_metric(
                    context=context, project_id=project_id, service=service, metric=metric, excluded_metrics_and_dimensions=excluded_metrics_and_dimensions
                )
