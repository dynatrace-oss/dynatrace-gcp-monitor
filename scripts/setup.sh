...
if [[ $(gcloud functions list --filter=$GCP_FUNCTION_NAME --project="$GCP_PROJECT" --format="value(name)")  ]]; then
  echo "- updating the function [$GCP_FUNCTION_NAME]"
else
  echo "- deploying the function [$GCP_FUNCTION_NAME]"
fi
