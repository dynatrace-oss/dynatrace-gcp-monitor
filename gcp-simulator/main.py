"""
App simulates some gcp endpoints
"""
import re
import time
...
async def services(
    project_id: str,
    state_filter: str = "state:ENABLED",
    page: Pagination = Depends(Pagination(50).update),
):
    if "DISABLED" in state_filter:
        return page.apply(
            {
                "services": [
...
async def projects(pagination: Pagination = Depends(Pagination(50).update)):
...
async def identity(account: str = ""):
    return Response(
...
async def time_series(
    ...
    for p in range(sub_project, SUB_PROJECTS):
        ...
        for i in range(instance, INSTANCES):
            ...
            res_dims = create_resource_dims(
                resource_labels, p, SUB_PROJECTS, i, project_id
            )
            for t in range(metric_tuple, requested_metric_tuples):
                ...
                metric_t = create_metric(metric_type, metric_labels, res_dims, t)
                ts_result_t.timeSeries.append(metric_t)

                limit, points = generate_points(
                    p, i, sample, resolution, start, end, limit
                )
